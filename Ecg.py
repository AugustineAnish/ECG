...
class ECG:
	...
	def DimensionalReduciton(self, test_final):
		...
		pca_loaded_model = joblib.load('Cardiovascular-Detection-using-ECG-images/Deployment/PCA_ECG.pkl')

		# Check if the loaded model is a PCA model
		if isinstance(pca_loaded_model, PCA):
			# Check if the number of features in the input data matches the PCA model's components
			if test_final.shape[1] == pca_loaded_model.n_components_:
				# Transform the input data using the loaded PCA model
				result = pca_loaded_model.transform(test_final)
				final_df = pd.DataFrame(result)
				return final_df
			else:
				return f"Error: Number of features in input data ({test_final.shape[1]}) does not match PCA components ({pca_loaded_model.n_components_})"
		else:
			return "Error: Loaded model is not a PCA model"
	def ModelLoad_predict(self, final_df):
		...
		loaded_model = joblib.load('Cardiovascular-Detection-using-ECG-images\Deployment\Heart_Disease_Prediction_using_ECG.pkl')

		# Check if the loaded model is valid
		if loaded_model is not None:
			# Perform any necessary preprocessing or transformation on the input data
			# Ensure that the input data format and shape match the model's requirements
			# For example, you may need to scale or encode the input data
			
			# Make predictions using the loaded model
			result = loaded_model.predict(final_df)

			# Process the prediction result and return the corresponding classification
			if result[0] == 1:
			...
			return "Error: Failed to load the pretrained model"
